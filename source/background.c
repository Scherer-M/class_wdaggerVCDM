...
int background_functions(
  ...
  /* Lambda */
    // ADF: here modified
  double ad_VCDM = pba->a_dagger_VCDM; // ADF: value of a_dagger
  double Delta_VCDM = pba->Delta_VCDM; // ADF: value of Delta
  double zeta_VCDM = pba->zeta_VCDM; // ADF: value of zeta
  double rho0_VCDM = pba->Omega0_lambda * pow(pba->H0,2);
  
  // ADF: next is rho_VCDM, or varrho_VCDM, i.e. in CLASS
  if (pba->has_lambda == _TRUE_) {
    // ADF: adding rhoX(a)
    //printf("\nCiao0: %lg %lg %lg %lg\n", ad_VCDM, Delta_VCDM, zeta_VCDM,rho0_VCDM);
    pvecback[pba->index_bg_rho_lambda] = rho0_VCDM * pow((pow(a / ad_VCDM, zeta_VCDM) + pow(ad_VCDM / a, zeta_VCDM)) / (pow(0.1e1 / ad_VCDM, zeta_VCDM) + pow(ad_VCDM, zeta_VCDM)), 0.3e1 * Delta_VCDM / zeta_VCDM);     rho_tot += pvecback[pba->index_bg_rho_lambda];
    //printf("\nCiao1: %lg\n", pvecback[pba->index_bg_rho_lambda]);
  } // ADF: as for now, no contribution to rho_tot and p_tot. Later on.
  ...
  // ADF: rho plus P: contribution from standard matter. Variable defined, needed for the perturbation equations. This variable include all matter except VCDM
  pvecback[pba->index_bg_rho_plus_P_VCDM] = rho_tot + p_tot;

  // ADF: only now adding rhoX and pX, pX is found by pX = w(a)*rhoX
  // ADF: pX = w(a) * rhoX
  double w_VCDM = -0.1e1 + Delta_VCDM * tanh(zeta_VCDM * log(ad_VCDM / a));
  //  printf("\nCiao2: %lg\n", w_VCDM);
  // Next is a*diff(w(a),a)
  double a_d_w_a_VCDM = -Delta_VCDM * zeta_VCDM * pow(0.1e1 / cosh(zeta_VCDM * log(ad_VCDM / a)), 0.2e1);
  double pX_VCDM = w_VCDM*pvecback[pba->index_bg_rho_lambda];
  p_tot += pX_VCDM;
  // ADF: Next is adding a*(d p_X / da), where pX is given above
  dp_dloga += pvecback[pba->index_bg_rho_lambda]*(a_d_w_a_VCDM -3.*w_VCDM*(1. + w_VCDM));
  //printf("\nCiao3: %lg %lg %lg %lg\n", w_VCDM, a_d_w_a_VCDM, pX_VCDM, dp_dloga);
...
int background_indices(
  ...
  class_define_index(pba->index_bg_rho_plus_P_VCDM,_TRUE_,index_bg,1);
...
int background_derivs(
  ...
  // ADF: sum over the relativistic species
  ...
  dy[pba->index_bi_D] = y[pba->index_bi_D_prime]/a/H; // ADF: dy -> dD/dloga
  // ADF: the next "commented" equation holds true also in VCDM in the high-k limit
  dy[pba->index_bi_D_prime] = -y[pba->index_bi_D_prime] + 1.5*a*rho_M*y[pba->index_bi_D]/H; // ADF: dy -> dD'/dloga
  // If high-k limit is fine uncomment previous line and comment the next one
  //dy[pba->index_bi_D_prime] = (0.3e1 * rho_M * H_prime / rho_plus_p / (-0.3e1 * H_prime * rho_M * a + k2_pivot_VCDM * rho_plus_p) / H * p_prime + (-0.27e2 * a * a * (H * H_prime * a + H_prime_prime) * rho_M * rho_M - 0.6e1 * H_prime_prime * rho_M * k2_pivot_VCDM - 0.2e1 * k2_pivot_VCDM * k2_pivot_VCDM * H * rho_plus_p) / (k2_pivot_VCDM + 0.9e1 / 0.2e1 * rho_M * a * a) / (-0.3e1 * H_prime * rho_M * a + k2_pivot_VCDM * rho_plus_p) / H / 0.2e1) * y[pba->index_bi_D_prime] + 0.3e1 * rho_M * (-0.3e1 * H_prime * rho_M * a + k2_pivot_VCDM * rho_plus_p) * a / (0.9e1 * rho_M * a * a + 0.2e1 * k2_pivot_VCDM) / rho_plus_p * y[pba->index_bi_D] / H;
  // ADF: end of changes
