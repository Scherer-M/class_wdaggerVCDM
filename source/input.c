...
int input_read_parameters(struct file_content * pfc,
  ...
  // ADF: parameters of VCDM which needs to be given
  class_read_double("a_dagger_VCDM",pba->a_dagger_VCDM);
  class_read_double("Delta_VCDM",pba->Delta_VCDM);
  class_read_double("zeta_VCDM",pba->zeta_VCDM);
...
int input_default_params(struct background *pba,
  ...
  // ADF: giving some default values for the VCDM parameters: Lambda-CDM case for CPL
  pba->a_dagger_VCDM = 0.6;
  pba->Delta_VCDM = 0.0;
  pba->zeta_VCDM = pow(10.0,1.5);
