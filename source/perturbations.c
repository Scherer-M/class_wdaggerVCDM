...
int perturbations_einstein(
  ...
  if (_scalars_) {
    ...
    if (ppt->gauge == newtonian) {
      ...
      // ADF: next is dH/d\eta
      double H_prime_VCDM = ppw->pvecback[pba->index_bg_H_prime];
      // ADF: next is sum_i (rho_i + p_i) and the sum covers all the standard matter fields
      double rho_plus_P_VCDM = ppw->pvecback[pba->index_bg_rho_plus_P_VCDM];
      /* equation for phi' */
      ppw->pvecmetric[ppw->index_mt_phi_prime] = -a_prime_over_a * ppw->pvecmetric[ppw->index_mt_psi]
	+ 3.*(k2-3.*a*H_prime_VCDM)/(k2*(2.*k2/a2+9.*rho_plus_P_VCDM))*ppw->rho_plus_p_theta;
